...
public class PatternRulesWithNdsTest extends BaseTest {

    @ParameterizedTest(name = "checkTruePatternRulesWithNds {0}")
    @CsvFileSource(resources = "/pattern-rules/PatternWithNds.csv")
    public void checkTruePatternRulesWithNds(String textPattern) {
        // create json with pattern
        String body = makeBodyFromMapForRuleRequest(textPattern, RULE_WITH_NDS);
    ...
    @ParameterizedTest(name = "checkFalsePatternRulesWithNds {0}")
    @CsvFileSource(resources = "/pattern-rules/PatternWithoutNds.csv")
    public void checkFalsePatternRulesWithNds(String textPattern) {
        // create json with pattern
        String body = makeBodyFromMapForRuleRequest(textPattern, RULE_WITH_NDS);
